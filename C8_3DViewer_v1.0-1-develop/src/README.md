...
![3DViewer](img/mainwindow.png)

## Внимание!

Программа в своей работе использует подключаемые библиотеки qt.  
Если они у вас отсутствуют, она может не запуститься.

### Сборка и тестирование.

Сборка и тестирование проводилось на следующей конфигурации:  
...
Перед сборкой проекта рекомендуется установить следующие компоненты (в соответствующем порядке):  

```
sudo apt-get update && sudo apt-get upgrade
sudo apt-get install make check lcov
sudo apt-get install build-essential
sudo apt-get install freeglut3-dev 
sudo apt-get install qtbase5-dev qtbase5-dev-tools 
sudo apt-get install qt5-qmake
```
\* qtcreator для сборки проекта не нужен, поэтому его можно не устанавливать.

### План Б.

В папке *gui* находится архивный файл *phoenix.tar.gz*, в котором есть готовый **makefile** и скомпилированная программа **3dViewer**. Однако, для корректной работы программы потребуются подключаемые библиотеки qt. 
