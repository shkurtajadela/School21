...
void GLWidget::drawObject() {
  if (object) {
    ...
    glDrawElements(GL_LINES, object->numbers_of_vertexes_in_facets,
                   GL_UNSIGNED_INT, object->single_array_for_facets);
...
void GLWidget::upload_file(QString filepath) {
  ...
  if (object) {
    ...
    if (parse_obj_file(file, object) &&
        allocate_memory_for_array_of_vertexes(file, object)) {
      object->numbers_of_vertexes_in_facets = 0;
      if (allocate_memory_for_array_vertexes_in_facets(file, object) &&
          allocate_memory_for_array_of_facets(file, object)) {
        ...
        for (unsigned int i = 0; i < object->numbers_of_vertexes_in_facets; i++)
          object->single_array_for_facets[i] =
              object->single_array_for_facets[i] - 1;
        update();
      }
    }
  }
...
void GLWidget::saveSettings() {
  if (line_type == "solid")
    this->settings.setValue("line_type", "solid");
  else
    this->settings.setValue("line_type", "dashed");
...
void GLWidget::loadSettings() {
  ...
  this->proection_type =
      settings.value("proection_type", "parallel").toString();
}